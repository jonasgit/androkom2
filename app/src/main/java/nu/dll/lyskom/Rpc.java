...
public interface Rpc {
	// errors
	/**
	 * No error has occurred. error-status is undefined. This should never
	 * happen, but it might.
	 */
	...
	/**
	 * A string was too long (see descriptions of each call.) error-status
	 ...
	 * despite this. error-status is undefined.
	 */
	...
	/**
	 * Attempt to access a non-existent or secret conference. error-status
	 * contains the conference number in question.
	 */
	public static final int E_undefined_conference = 9;

	/**
	 * Attempt to access a non-existent or secret person. error-status contains
	 * the person number in question.
	 */
	 ...
	 * conference without enough permission to do so. error-status indicates the
	 * object to which we didn't have enough permissions to.
	 */
	public static final int E_access_denied = 11;

	/**
	 * Not enough permissions to do something. The exact meaning of this
	 * response depends on the call. error-status indicated the object for which
	 ...
	 * caller is not a member of. error-status indicates the conference in
	 * question.
	 */
	 ...
	 * way. error-status indicates the text number in question.
	 */
	 ...
	 * represent an existing text. error-status indicates the offending number.
	 */
	 ...
	 * characters. error-status is undefined.
	 */
	public static final int E_bad_name = 18;

	/**
	 * Attempt to use a number that's out of range. The range and meaning of the
	 * numbers depends on the call issued. error-status is undefined unless
	 * stated otherwise in the call documentation.
	 */
	public static final int E_index_out_of_range = 19;

	/**
	 * Attempt to create a conference or person with a name that's already
	 * occupied. error-status is undefined.
	 */
	public static final int E_conference_exists = 20;

	/**
	 * Attempt to create a person with a name that's already occupied.
	 * error-status is undefined. This error code is probably not used, but you
	 * never know for sure.
	 */
	public static final int E_person_exists = 21;

	/**
	 * Attempt to give a conference a type with secret bit set and the rd-prot
	 * bit unset. This is an error since such a conference type is inconsistent.
	 * error-status is undefined.
	 */
	public static final int E_secret_public = 22;

	/**
	 * Attempt to change the letterbox flag of a conference. error-status
	...
	/**
	 * Attempt to create an illegal misc item. error-status contains the index
	 * of the illegal item.
	 */
	 ...
	 * knows nothing about. error-status is the type.
	 */
	public static final int E_illegal_info_type = 26;

	/**
	 * Attempt to add a recipient that is already a recipient of the same type.
	 * error-status contains the recipient that already is.
	 */
	public static final int E_already_recipient = 27;

	/**
	 * Attempt to add a comment to a text twice over. error-status contains the
	 * text number of the text that already is a comment.
	 */
	public static final int E_already_comment = 28;

	/**
	 * Attempt to add a footnote to a text twice over. error-status contains the
	 * text number of the text that already is a footnote.
	 */
	public static final int E_already_footnote = 29;

	/**
	 * Attempt to remove a recipient that isn't really a recipient. error-status
	 * contains the conference number in question.
	 */
	public static final int E_not_recipient = 30;

	/**
	 * Attempt to remove a comment link that does not exist. error-status
	 * contains the text number that isn't a comment.
	 */
	public static final int E_not_comment = 31;

	/**
	 * Attempt to remove a footnote link that does not exist. error-status
	 ...
	 * of recipients. error-status is the text that has the maximum number of
	 * recipients.
	 */
	public static final int E_recipient_limit = 33;

	/**
	 * Attempt to add a comment to a text that already has the maximum number of
	 * comments. error-status is the text with the maximum number of comments.
	 */
	public static final int E_comment_limit = 34;

	/**
	 * Attempt to add a footnote to a text that already has the maximum number
	 * of footnote. error-status is the text with the maximum number of
	 * footnotes.
	 */
	public static final int E_footnote_limit = 35;

	/**
	 * Attempt to add a mark to a text that already has the maximum number of
	 * marks. error-status is the text with the maximum number of marks.
	 */
	public static final int E_mark_limit = 36;

	/**
	 * Attempt to manipulate a text in a way that required the user to be the
	 * author of the text, when not in fact the author. error-status contains
	 * the text number in question.
	 */
	 ...
	 * response is sent when attempting to delete an item set by someone else or
	 * an item that can't be deleted, and when attempting to create an item
	 * without permissions to do so. error-status contains the index at which
	 * the item appears in the aux-item list sent to the server.
	 */
	public static final int E_aux_item_permission = 49;

	/**
	 * Sent in response to a request for an asynchronous message the server does
	 * not send. The call succeeds, but this is sent as a warning to the client.
	 * error-status contains the message type the server did not understand.
	 */
	...
	/**
	 * Attempt to use a feature that has been explicitly disabled in the server.
	 * error-status is undefined.
	 */
	 ...
	 * viable recipients for a group message. error-status is undefined.
	 */
	public static final int E_message_not_sent = 53;

	/**
	 * A requested membership type was not compatible with restrictions set on
	 * the server or on a specific conference. error-status is undefined unless
	 * specifically mentioned in the documentation for a specific call.
	 */
