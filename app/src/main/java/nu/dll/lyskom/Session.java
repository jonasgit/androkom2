 ...
 * returns immediately. The call will be written to the server by another
 * thread.
 * </p>
 * <p>
 * An application may register itself as an <tt>RpcEventListener</tt>, and send
 * commands to the server by calling the <tt>do...()</tt> methods. LatteKOM will
 * then call the the <tt>rpcEvent(RpcEvent)</tt> method when a reply arrives
 * from the server. The application may also use the <tt>waitFor(int)</tt>,
 ...
 * <tt>do</tt>" prefix. They also add the extra convenience of interpreting the
 * RPC reply and returning objects that are easier to deal with. For example,
 ...
 * by the server. If a synchronous call fails, a <tt>RpcFailure</tt> exception
 * should be thrown containing information about the error that occured.
 * </p>
 * <p>
 * Asynchronous messages can be received by clients by registering
 * <tt>AsynchMessageReceiver</tt> objects with the
 * <tt>addAsynchMessageReceiver</tt> method. The objects'
 * <tt>asynchMessage()</tt> method will be called by the network read thread at
 * the moment a message has been received and parsed into an
 ...
 * messages will be read during the execution of the receiver's method. Most
 * notably, the receiving method must not send an RPC call and then call the
 * waitFor() method to wait for the reply (this will cause an
 * <tt>IOException</tt> to be thrown by the waitFor() method).
 * </p>
 * <p>
 * The LatteKOM library keeps caches of texts, conferences, persons and
 * membership information. The caches are refreshed or purged when the server
 * indicates that it might be needed. At the moment, much of that functionality
 * is still in development and might therefore be inefficient, incomplete or
 * buggy (that goes for large parts of LatteKOM in general).
 * </p>
 * <p>
 * A very simple program to login and read a specific LysKOM text could look
 * like this:
 * 
 * <pre>
 ...
 * @version $Id: Session.java,v 1.96 2010/05/29 20:00:15 pajp Exp $
...
public class Session implements AsynchMessageReceiver, RpcReplyReceiver,
        RpcEventListener {

    /**
     * Time-out used by the waitFor() method in milliseconds. Can be set (in
     * seconds) through the system property "lyskom.rpc-timeout". Default is 30
     * seconds.
     */
    public static int rpcTimeout;

    /**
     * This is the "soft" timeout, in milliseconds. If larger than zero, the
     * waitFor() call will only block for this long, and then check if a reply
     * has been received even if the receiver thread hasn't notified yet. If
     * none found, it will repeat the process until a reply is found or
     * rpcTimeout has been reached. <br>
     * Default is zero, i.e., no soft timeout will be used, and the waitFor()
     * method will block until it is notified by the receiving thread. It can be
     * set (in seconds) by the system property "lyskom.rpc-soft-timeout".
     */
    public static int rpcSoftTimeout;

    /**
     * The encoding used by the LysKOM server. All conversions from bytes to
     * String (and vice versa) in server I/O should respect this setting. It can
     * be changed by setting the system property "lyskom.encoding". Default is
     * "iso-8859-1".
     */
    ...
    // set property values
    static {
        defaultServerEncoding = System.getProperty("lyskom.encoding",
                "iso-8859-1");
        rpcTimeout = Integer.getInteger("lyskom.rpc-timeout", 60).intValue() * 1000;
        rpcSoftTimeout = Integer.getInteger("lyskom.rpc-soft-timeout", 0)
                .intValue() * 1000;
        defaultBigTextLimit = Integer.getInteger("lyskom.big-text-limit",
                150 * 1024).intValue();
        defaultBigTextHead = Integer.getInteger("lyskom.big-text-head", 100)
                .intValue();
        defaultEnabledBigText = Boolean.getBoolean("lyskom.big-text");
        defaultLazyTextLimit = Integer.getInteger("lyskom.lazy-text-limit", 80)
                .intValue();
    }
    ...
    /**
     * Connected to a LysKOM server, but not logged in. In this state only a few
     * calls can be made to the server (lookupName(), login(), create(), etc.)
     */
    ...
    Map<Integer, SessionInfo> sessionCache;
    ...
    Vector<RpcEventListener> rpcEventListeners;
    ...
    List<AsynchMessage> pendingAsynchMessages = new LinkedList<AsynchMessage>();
    ...
    private void init() {
        ...
        sessionCache = new HashMap<Integer, SessionInfo>();
        readTexts = new ReadTextsMap();
        rpcHeap = new RpcHeap();
        rpcEventListeners = new Vector<RpcEventListener>(1);
        mainThread = Thread.currentThread();
        textPrefetchQueue = new LinkedList<Integer>();
    ...
    boolean dontCacheBinaries = Boolean
            .getBoolean("lattekom.dont-cache-binaries");
    ...
    /**
     * Removes any evidence of the given text in the caches (Text cache and
     * Text-Stat cache)
     */
    ...
    /**
     * Adds RPC reply listeners. These will be called through the
     * RpcEventListener (rpcEvent()) interface when RPC replies are received.
     * 
     * @see nu.dll.lyskom.RpcEventListener
     * @see nu.dll.lyskom.RpcEvent
     */
    public void addRpcEventListener(RpcEventListener l) {
        rpcEventListeners.addElement(l);
    }
    ...
    public void removeRpcEventListener(RpcEventListener l) {
        rpcEventListeners.removeElement(l);
    }

    /**
     * Traverses through the rpcEventListeners vector and calls all registered
     * event listeners.
     * 
     * This method could probably be private.
     */
    protected void notifyRpcEventListeners(RpcEvent ev) {
        for (Enumeration<RpcEventListener> e = rpcEventListeners.elements(); e
                .hasMoreElements();)
            e.nextElement().rpcEvent(ev);
    }

    /**
     * Disconnects the given session from the server, given appropriate
     * permissions.
     */
    public RpcCall doDisconnect(int sessionNo, boolean discardReply)
            throws IOException {
        return writeRpcCall(
                new RpcCall(count(), Rpc.C_disconnect).add(new KomToken(
                        sessionNo)), !discardReply);
    }
     ...
     * permissions.
     */
    public void disconnect(int sessionNo) throws IOException, RpcFailure {
     ...
     * 
     * @param force
     *            (unused) Do not wait for the server to reply to the disconnect
     *            command
     */
    ...
    public String getCanonicalName() throws IOException, RpcFailure {
        Map<String, Object> info = getInfo();
        @SuppressWarnings("unchecked")
        AuxItem item = AuxItem.getFirst(AuxItem.tagCanonicalName,
                (List<AuxItem>) info.get("aux-item-list"));
        return item != null ? item.getDataString() : null;
    }

    public String getServerName() throws IOException, RpcFailure {
     ...
     * 
     * @param id
     *            ID number of the person to log in as
     * @param password
     *            corresponding password, will be converted to bytes according
     *            to server encoding
     * @param hidden
     ...
     *            if true, session will not be broadcasted on LysKOM
     * @param getMembership
     *            if true, the users membership will be queried upon login
     * 
     */
    public boolean login(int id, String password, boolean hidden,
     ...
     */
    public Person getMyPerson() throws IOException, RpcFailure {
        if (myPerson == null) {
    ...
    /**
     * Returns the current cached list of unread conferences. If neither
     * getUnreadConfsList() nor updateUnreads() has been called, this list will
     * be empty.
     * 
     */
    public List<Integer> getUnreadConfsListCached() {
        return unreadsConferences;
    }

    /**
     * Returns the current cached Membership of the given conference. If the
     * Membership for the given conference hasn't yet been queries,
     * <tt>null</tt> is returned. Calling updateUnreads() before guarantees that
     * Membership has been queried for all conferences in the Unread-Conferences
     * list returned by getUnreadConfsList[Cached]().
     * 
     */
     ...
     */
    public void updateUnreads() throws IOException {
    ...
    /**
     * Updates the unreadMembership array with Membership objects for
     * conferences that may contain unreads. If the given parameter is non-null,
     * it will be used as the starting point for which conferences are queries,
     * and the same list will be returned by subsequent calls to
     * getUnreadConfsListCached(). If null, getUnreadConfsList() will be called
     * to retrieve that data instead.
     */
    public void updateUnreads(List<Integer> _unreads, boolean getReadTexts,
            int minPrio) throws IOException {
        ...
        if (_unreads == null) {
            ...
            unreadsConferences = _unreads;
        }

        unreadMembership = new LinkedList<Membership>();

        // first, bulkly send all get-uconf-stat and query-read-texts
        // calls to the server
        List<Integer> pendingCalls = new LinkedList<Integer>();
        synchronized (unreadsConferences) {
            for (Iterator<Integer> i = unreadsConferences.iterator(); i.hasNext();) {
                int conf = i.next().intValue();
                pendingCalls.add(new Integer(doGetUConfStat(conf).getId()));
                pendingCalls.add(new Integer(doQueryReadTexts(persNo, conf)
                        .getId()));
            }
        }
        ...
        while (pendingCalls.size() > 0) {
            ...
            if (rc.getOp() == Rpc.C_get_uconf_stat) {
                conferenceCache.add(new UConference(rc.getParameter(0)
                        .intValue(), rc.getReply().getParameters()));
            } else if (rc.getOp() == Rpc.C_query_read_texts
                    || rc.getOp() == Rpc.C_query_read_texts_10) {
                membershipCache.add(Membership.createFrom(0, rc.getReply()
                        .getParameters(),
                        rc.getOp() == Rpc.C_query_read_texts_10));
            } else {
                throw new RuntimeException("Unexpected RPC reply " + rc.getOp());
            }
        ...
        synchronized (unreadsConferences) {
            List<Integer> unreadsCopy = new LinkedList<Integer>();
            unreadsCopy.addAll(unreadsConferences);
            unreadsConferences.clear();
            for (int i = 0; i < unreadsCopy.size(); i++) {
                ...
                if (m.getPriority() < minPrio) {
                    Debug.println("updateUnreads(): skipping conf " + conf
                            + " with prio " + m.getPriority());
                    continue;
                }

                int possibleUnreads = getUConfStat(m.getNo())
                        .getHighestLocalNo() - m.getLastTextRead();
                possibleUnreads -= m.getReadTexts().length;

                if (possibleUnreads > 0) {
                    unreadsConferences.add(new Integer(conf));
                    unreadMembership.add(m);
                } else {
                    Debug.println("updateUnreads(): skipping empty conf "
                            + conf);
                }
            }
        }
    ...
    /**
     * Marks the text as read in all recipient conferences which the user is a
     * member of.
     * 
     */
    ...
    public void markAsRead(int textNo) throws IOException, RpcFailure {
        ...
        while (recipientIterator.hasNext()) {
            ...
            if (rcpt > 0 && isMemberOf(rcpt)) {
                ...
                Debug.println("markAsRead: global " + textNo + " rcpt " + rcpt
                        + " local " + local);
    ...
    public int nextUnreadConference(boolean change) throws IOException {
                ...
                if (iter.hasNext()) {
                    ...
                    if (change) {
                        ...
                        if (nextUnreadText(false) == -1) {
                            unreadsConferences.remove(new Integer(currentConference));
        ...
        if (change) {
            changeConference(((Integer) unreadsConferences.get(0)).intValue());
            if (oldCurrent == currentConference) // prolly buggy
                return -1;
            return currentConference;
        } else {
            return ((Integer) unreadsConferences.get(0)).intValue();
        }
    }

    public int nextUnreadText(int conference, boolean updateUnread)
            throws IOException {
        List<Integer> list = nextUnreadTexts(conference, updateUnread, 1);
        if (list.size() == 0)
            return -1;
    ...
    /**
     * Returns the global text number of the next unread text in the current
     * conference. Returns -1 if there are no unread texts.
     * 
     ...
     */
    public List<Integer> nextUnreadTexts(int conference, boolean updateUnread,
            int maxTexts) throws IOException {
        return nextUnreadTexts(conference, updateUnread, maxTexts, true);
    }

    public List<Integer> nextUnreadTexts(int conference, boolean updateUnread,
            int maxTexts, boolean modifyUnreadList) throws IOException {
        ...
        if (c.getHighestLocalNo() > m.getLastTextRead()) {
            ...
            if (!tm.hasMoreElements()) {
                if (modifyUnreadList) {
                    synchronized (unreadsConferences) {
                        unreadsConferences.remove(new Integer(conference));
                    }
            ...
            if (updateUnread) {
                ...
                for (Iterator<Integer> i = returnList.iterator(); i.hasNext();) {
                    int[] ur = { tm.localToGlobal(((Integer) i.next())
                            .intValue()) };
                    markAsRead(conference, ur);
                }
            }
            lastText = ((Integer) returnList.get(returnList.size() - 1))
                    .intValue();
            return returnList;
        } else {
            if (modifyUnreadList)
    ...
    /**
     * Same as
     * <tt>nextUnreadText(session.getCurrentConference(), <i>updateUnread</i>)</tt>
     * .
     * 
     * @param updateUnread
     ...
     */
    public int nextUnreadText(boolean updateUnread) throws IOException {
     ...
     * 
     * If the membership list hasn't been read from the server, it will be
     * fetched if "fetchMembership" is true. If it is false, this method will
     * return <tt>false</tt>.
     * 
     * @param confNo
     *            Conference number
     */
    public boolean isMemberOf(int confNo, boolean fetchMembership)
            throws IOException {
        if (memberships == null) {
            if (fetchMembership) {
        ...
        Iterator<Membership> i = memberships.iterator();
        while (i.hasNext()) {
     ...
     *            Conference number
     */
    public int getUnreadCount(int confNo) throws IOException {
        if (unreadsConferences.size() == 0 || !unreadsConferences.contains(new Integer(confNo)))
            return 0;
    ...
    /**
     * Returns a ReadTextsMap object. The ReadTextsMap is a set containing all
     * texts read during this session.
     */
    public ReadTextsMap getReadTexts() {
        return readTexts;
    }

    /**
     * Marks a text as read on the LysKOM server. Note that the text number is
     * local.
     * 
     * @param confNo
     *            conference number
     ...
     */
    public void markAsRead(int confNo, int[] localTextNo) throws IOException {
        ...
        for (int i = 0; i < localTextNo.length; i++) {
            ...
            Debug.println("marked local " + localTextNo[i] + " in conf "
                    + confNo + " as read");
        }
    }

    /**
     * Returns the client version (as a byte array) for a given session. Returns
     * a zero length array of the client has no version string set.
     * 
     * @param sessionNo
     *            The session for which to retreive the client version.
     * @see nu.dll.lyskom.Session#getClientName(int)
     */
    public byte[] getClientVersion(int sessionNo) throws IOException,
            RpcFailure {
     ...
     */
    public RpcCall doGetClientVersion(int sessionNo) throws IOException {
        return writeRpcCall(new RpcCall(count(), Rpc.C_get_client_version)
                .add(new KomToken(sessionNo)));
    }

    /**
     * Returns the client name (as a byte array) for a given session. Returns a
     * zero length array of the client has no name string set.
     * 
     * @param sessionNo
     *            The session for which to retreive the client name.
     * @see nu.dll.lyskom.Session#getClientVersion(int)
     */
    public byte[] getClientName(int sessionNo) throws IOException, RpcFailure {
     ...
     * @see nu.dll.lyskom.Session#getClientName(int)
     */
    public RpcCall doGetClientName(int sessionNo) throws IOException {
     ...
     * 
     * The keys in the returned map corresponds to the fields specified in the
     * `Info' data type in the LysKOM protocol A specification.
     * 
     * All values in the map are stores as KomToken object, with the exception
     * of "aux-item-list", which is a List of AuxItem objects.
     * 
     * This information is persistently cached for the duration of the session.
     * 
     */
    public Map<String, Object> getInfo() throws IOException, RpcFailure {
        if (serverInfo != null)
            return serverInfo;
        ...
        List<AuxItem> auxItems = new LinkedList<AuxItem>();
        for (int i = 0; i < auxListLength; i++) {
            AuxItem item = new AuxItem(
                    ((KomTokenArray) data[offset]).getTokens());
            if (Debug.ENABLED)
                Debug.println("Server aux-item: " + item);
            auxItems.add(item);
    ...
    public List<String> getAllowedContentTypes() throws IOException, RpcFailure {
        List<?> auxItems = (List<?>) getInfo().get("aux-item-list");
        List<String> allowedContentTypes = new LinkedList<String>();
    ...
    public RpcCall doGetInfo() throws IOException {
        return writeRpcCall(new RpcCall(count(), Rpc.C_get_info));
    }

    /**
     * Returns static session information for a given session number. This is
     * guaranteed by the LysKOM protocol to never change for a session number
     * during a server's life time.
     * 
     * @param sessionNo
     *            The session for which to retreive information
     * @return A <tt>SessionInfo</tt> object containing static session
     *         information
     * @see nu.dll.lyskom.SessionInfo
     */
    public SessionInfo getStaticSessionInfo(int sessionNo) throws IOException,
            RpcFailure {
        SessionInfo session;
     ...
     */
    public RpcCall doGetStaticSessionInfo(int sessionNo) throws IOException {
        return writeRpcCall(new RpcCall(count(), Rpc.C_get_static_session_info)
                .add(new KomToken(sessionNo)));
    }

    /**
     * Sets client name and version, prepending the supplied strings with a
     * slash and the name and version of the LatteKOM library, respectively. The
     * resulting name and version would then be something like
     * <tt>"LatteKOM/Swing"</tt>, <tt>"0.1/0.9"</tt>
     * 
     * @param clientName
     *            The name of the LysKOM client
     * @param clientVersion
    ...
    public void setLatteVersion(String clientName, String clientVersion)
        ...
        setClientVersion(latteName + "/" + clientName, latteVersion + "/"
                + clientVersion);
    }
    ...
    /**
     * Reports the name and version of this client to the server. These can be
     * retreived by other clients by the calls <tt>get-client-name</tt> and
     * <tt>get-client-version</tt>. I recommend using the
     * <tt>setLatteVersion(String, String)</tt> method instead, to also report
     * name and version of the LatteKOM library.
     * 
     * @param clientName
     *            The name of the LysKOM client
     * @param clientVersion
    ...
    public RpcCall doSetClientVersion(String clientName, String clientVersion)
            throws IOException {
        return writeRpcCall(new RpcCall(count(), Rpc.C_set_client_version).add(
                new Hollerith(clientName)).add(new Hollerith(clientVersion)));
    }
     ...
     * 
     * @param confNo
     *            the conference in which to mark texts as read
     * @param localTextNo
     *            an <tt>int</tt> array containing the local text numbers to
     *            mark as read
     * @return An RpcCall object representing this specific RPC call
     */
    public synchronized RpcCall doMarkAsRead(int confNo, int[] localTextNo)
        ...
        req.add(new KomToken(confNo)).add(
                new KomTokenArray(localTextNo.length, localTextNo));
        writeRpcCall(req);
        return req;
    }

    public synchronized RpcCall doGetTime() throws IOException {
        RpcCall req = new RpcCall(count(), Rpc.C_get_time);
        writeRpcCall(req);
        return req;
    }

    public KomTime getTime() throws IOException {
     ...
     * If a cached copy of the text exists, it will be returned instead.
     * 
     * @param textNo
     *            Global text number
     * @see nu.dll.lyskom.Session#getText(int, boolean)
     */
    public Text getText(int textNo) throws IOException, RpcFailure {
     ...
     *            Global text number
     * @param refreshCache
     *            If <tt>true</tt>, this call will never return a cached copy of
     *            a text.
     ...
     *            a text.
     * @param useLazyText
     *            If <tt>true</tt> the text contents exceeding the
     *            lazy-text-limit will not be read until getContents() is called
     * @see nu.dll.lyskom.Session#getText(int)
     * 
     */
    public synchronized Text getText(int textNo, boolean refreshCache,
        ...
        if (enableBigText && textIsBig) {
            ...
            if (useLazyText) {
                ...
                contentLimit = textStat.getSize() > lazyTextLimit ? lazyTextLimit
                        : textStat.getSize();
            } else {
        ...
        if (isCachableType(text.getContentType())) {
            ...
            if (Debug.ENABLED) {
                Debug.println("Not caching " + text + " ("
                        + text.getContentType() + ")");
            }
        }
        return text;
     ...
     * 
     * @param textNo
     *            the text number to mark
     * @param markType
     *            the type of mark to set
     * @return An RpcCall object representing this specific RPC call
     */
    public RpcCall doMarkText(int textNo, int markType) throws IOException {
    ...
    /**
     * Place a personal mark on a LysKOM text. Blocks until the server has
     * replied.
     * 
     * @param textNo
     *            the text number to mark
     ...
     *            the type of mark to set
     */
    public void markText(int textNo, int markType) throws IOException,
            RpcFailure {
     ...
     * @return An RpcCall object representing this specific RPC call
     */
    public RpcCall doUnmarkText(int textNo) throws IOException {
     ...
     *            the text number to unmark
     */
    public void unmarkText(int textNo) throws IOException, RpcFailure {
     ...
     * @return An RpcCall object representing this specific RPC call
     */
    public RpcCall doGetMarks() throws IOException, RpcFailure {
     ...
     */
    public Mark[] getMarks() throws IOException, RpcFailure {
     ...
     * 
     * See the "<tt>local-to-global</tt>" RPC call and the "
     ...
     */
    Map<String, SoftReference<TextMapping>> ltgCache = new HashMap<String, SoftReference<TextMapping>>();

    public TextMapping localToGlobal(int confNo, int firstLocalNo,
            int noOfExistingTexts) throws IOException, RpcFailure {
        if (firstLocalNo == 0)
            throw new IllegalArgumentException(
                    "First local text number cannot be zero.");
        ...
        do {
            ...
            if (!r.getSuccess())
                throw new RpcFailure(r, "in localToGloal(" + confNo + ", "
     ...
     * @param confNo
     *            The conference number in which to query information about read
     *            texts
     * @return An RpcCall object representing this specific RPC call
     */
    public RpcCall doQueryReadTexts(int persNo, int confNo) throws IOException {
        RpcCall req;
    ...
    /**
     * Returns a Membership object containing information about read/unread
     * texts for a given person in a given conference.
     * 
     * @param persNo
     *            The person number for which to query information
     * @param confNo
     ...
     */
    public Membership queryReadTexts(int persNo, int confNo) throws IOException {
     ...
     *            texts
     * @param refresh
     *            If <tt>true</tt>, the membership cache will be refreshed
     * @see nu.dll.lyskom.Membership
     * @see nu.dll.lyskom.Session#queryReadTexts(int, int, boolean)
     */
     ...
     */
    public RpcCall doGetUnreadConfs(int persNo) throws IOException {
     ...
     * @see nu.dll.lyskom.Session#getUnreadConfs(int)
     */
    public List<Integer> getUnreadConfsList(int persNo) throws IOException {
        return getUnreadConfsList(persNo, false);
    }

    /**
     * Convenience wrapper for getUnreadConfsList(). Uses the logged-in user ID.
     * 
     * @see #getUnreadConfsList(int)
     */
    public List<Integer> getMyUnreadConfsList() throws IOException {
     ...
     */
    public List<Integer> getMyUnreadConfsList(boolean askServer)
            throws IOException {
     ...
     *            The person number for which to query information
     * @param askServer
     *            if true, do not use cached information
     * @see nu.dll.lyskom.Session#getUnreadConfs(int)
     */
    public List<Integer> getUnreadConfsList(int persNo, boolean askServer)
            throws IOException {
        if (persNo == myPersonNo && !askServer && unreadsConferences != null) {
            return unreadsConferences;
        }
        LinkedList<Integer> confList = new LinkedList<Integer>();
        int[] confs = getUnreadConfs(persNo);
        for (int i = 0; i < confs.length; i++)
            confList.add(new Integer(confs[i]));
        if (persNo == myPersonNo)
            unreadsConferences = confList;
        return confList;
    }

    /**
     * Returns an <tt>int</tt> array containing all conferences which may
     * contain unread texts.
     * 
     * @param persNo
     *            The person number for which to query information
     * @see nu.dll.lyskom.Session#getUnreadConfsList(int)
     */
    public int[] getUnreadConfs(int persNo) throws IOException {
     ...
     *            The person number for which to query information
     * @param first
     *            The first membership position in list to retreive
     * @param no
     *            The number of conferences to retreive
     * @param wantReadTexts
     *            If true, the server will return a read-texts array with the
     *            membership information
     * @return An RpcCall object representing this specific RPC call
     */
    public RpcCall doGetMembership(int persNo, int first, int no,
            boolean wantReadTexts) throws IOException {
        RpcCall req;
        if (proto_10_membership) {
            req = new RpcCall(count(), Rpc.C_get_membership_10)
                    .add(new KomToken(persNo)).add(new KomToken(first))
                    .add(new KomToken(no)).add(new KomToken(wantReadTexts));
        } else {
     ...
     */
    public RpcCall doGetMembership(int persNo) throws IOException {
        return doGetMembership(persNo, 0, 1000, false);
    }

    /**
     * Equal to
     * <tt>getMembershipList(myPerson.getNo(), 0, myPerson.noOfConfs+1, new Bitstring("0"))</tt>
     * 
    ...
    /**
     * Returns a List of Membership objects representing the membership
     ...
     * 
     */
    public List<Membership> getMembershipList(int persNo, int first, int no,
            boolean wantReadTexts) throws IOException {
        Debug.println("getMembershipList(" + persNo + ", " + first + ", " + no
                + ", " + wantReadTexts + ")");
        ...
        Debug.println("getMembershipList(persNo:" + persNo + "): returning "
                + l.size() + " confererences");
        return l;
    }

    /**
     * Returns an array of Membership objects representing the membership
     * information for a given person.
     * 
     * @param persNo
     *            The person number for which to query information
     ...
     *            The number of conferences to retreive
     * @param wantReadTexts
     *            If the first bit is set, the server will not return a
     *            read-texts array with the membership information
     */
    public Membership[] getMembership(int persNo, int first, int no,
            boolean wantReadTexts) throws IOException {
        RpcCall call = waitForCall(doGetMembership(persNo, first, no,
                wantReadTexts));
        if (!call.getReply().getSuccess())
            throw call.getReply().getException();
        ...
        Arrays.sort(memberships, new Comparator<Membership>() {
     ...
     * 
     * @param name
     *            The requested name of the person
     * @param password
     *            The requested password for this new person
     * @param flags
     *            A Bitstring of flags
     * @param auxItems
     *            an array of AuxItem objects that should be set for this person
     * @return An RpcCall object representing this specific RPC call
     * @see nu.dll.lyskom.Session#createPerson(String, String, Bitstring,
     *      AuxItem[])
     */
    public RpcCall doCreatePerson(String name, String password,
            Bitstring flags, AuxItem[] auxItems) throws IOException {
     ...
     *      AuxItem[])
     */
    public int createPerson(String name, String password, Bitstring flags,
            AuxItem[] auxItems) throws IOException, RpcFailure {
    ...
    public RpcCall doFindPreviousTextNo(int text) throws IOException {
     ...
     * 
     * @param name
     *            The requested name of the confernece
     * @param flags
     *            A Bitstring of flags
     * @param auxItems
     *            an array of AuxItem objects that should be set for this
     *            conference
     * @return An RpcCall object representing this specific RPC call
     ...
     * @param name
     *            The requested name of the conference
     * @param readProt
     *            if <tt>true</tt>, the new conference should be read protected
     * @param original
     *            if <tt>true</tt>, the new conference should not allow comments
     * @param secret
     *            if <tt>true</tt>, the new conference should be secret
     */

    public int createConf(String name, boolean readProt, boolean original,
            boolean secret) throws IOException {
        Bitstring confType = new Bitstring(new boolean[] { readProt, original,
                secret, false });
        RpcReply r = waitFor(doCreateConf(name, confType, new AuxItem[] {})
                .getId());
        if (r.getSuccess()) {
            return r.getParameters()[0].intValue();
        }
     ...
     */
    public Membership[] getMembership(int persNo) throws IOException {
    ...
    /**
     * Returns an array of Membership objects, representing the conferences in
     ...
     */
    public RpcCall doGetUConfStat(int confNo) throws IOException {
    ...
    /**
     * Returns an <tt>UConference</tt> object containing information about a
     * specific conference.
     * 
     * @param confNo
     *            The conference to request information about
     * @param refreshCache
     *            if <tt>true</tt>, don't look in the cache
    ...
    /**
     * Returns an <tt>UConference</tt> object containing information about a
     * specific conference. Returns a cached copy if possible.
     * 
     * @param confNo
     *            The conference to request information about
     * @see nu.dll.lyskom.UConference
     */
    public UConference getUConfStat(int confNo) throws IOException, RpcFailure {
        return getUConfStat(confNo, false);
    }

    public void saveUserArea(UserArea userArea) throws RpcFailure, IOException {
        saveUserArea(myPersonNo, userArea);
    }

    public void saveUserArea(int persNo, UserArea userArea) throws RpcFailure,
            IOException {
        Text userDataText = new Text(userArea.toNetwork());
        userDataText.setCharset(userArea.getCharset());
        userDataText.getStat().setAuxItem(
                new AuxItem(AuxItem.tagContentType, UserArea.contentType));
    ...
    public RpcCall doSetUserArea(int persNo, int userAreaTextNo)
        ...
        return writeRpcCall(new RpcCall(count(), Rpc.C_set_user_area).add(
                new KomToken(persNo)).add(new KomToken(userAreaTextNo)));
    }

    public void setUserArea(int persNo, int userAreaText) throws RpcFailure,
            IOException {
    ...
    public void setUserArea(int userAreaText) throws IOException, RpcFailure {
        setUserArea(myPersonNo, userAreaText);
    }

    public UserArea getUserArea() throws RpcFailure, IOException {
        int userAreaTextNo = getMyPerson().getUserArea();
        if (userAreaTextNo == 0)
            return new UserArea(getServerEncoding());
    ...
    /**
     * Sets the maximum number of unread texts in the current conference. Note
     * that this call will fail if the user is not currently in a conference.
     * 
     * @param no
     *            The number of unread texts to request
     * @see nu.dll.lyskom.Session#endast(int, int)
     */
    public void endast(int no) throws IOException, RpcFailure {
     ...
     * 
     * @param confNo
     *            The conference in which to set the number of unread texts
     * @param no
     *            The number of unread texts to request
     * @see nu.dll.lyskom.Session#endast(int)
     */
    public void endast(int confNo, int no) throws IOException, RpcFailure {
        ...
        if (unreadsConferences != null) {
            synchronized (unreadsConferences) {
                if (!unreadsConferences.contains(new Integer(confNo))) {
                    if (lastRead < highest)
                        unreadsConferences.add(new Integer(confNo));
                }
            }
        }
        readTexts.clear();
     ...
     * 
     * @param confNo
     *            The conference to set last-read information in
     * @param textNo
     *            the text number to be the highest read text
     * @return An RpcCall object representing this specific RPC call
     * @see nu.dll.lyskom.Session#endast(int, int)
     */
    public RpcCall doSetLastRead(int confNo, int textNo) throws IOException {
        return writeRpcCall(new RpcCall(count(), Rpc.C_set_last_read).add(
                new KomToken(confNo)).add(new KomToken(textNo)));
    }

    /**
     * Sets the highest local text number that has been read in a given
     * conference.
     * 
     * @param confNo
     *            The conference to set last-read information in
     ...
     * 
     */
    public void setLastRead(int confNo, int textNo) throws IOException {
    ...
    public Conference getConfStat(int confNo, boolean refreshCache)
            throws IOException, RpcFailure {
        Conference cc = refreshCache ? null : conferenceCache
                .getConference(confNo);
        if (cc != null)
            return cc;
     ...
     */
    public void changeConference(int confNo) throws IOException, RpcFailure {
        if (confNo == currentConference)
            return;
     ...
     * @return An RpcCall object representing this specific RPC call
     */
    public RpcCall doChangeConference(int confNo) throws IOException {
        RpcCall req = new RpcCall(count(), Rpc.C_change_conference).add(confNo);
        writeRpcCall(req);
        return req;
    }
     ...
     * 
     * @param confNo
     *            The conference to remove a member from
     * @param persNo
    ...
    /**
     * Removes the person <tt>persNo</tt> from conference <tt>confNo</tt>. Only
     * the supervisor of <tt>confNo</tt> or <tt>persNo</tt> can do this. Also
     * sets the current conference to be -1 if the call succeeds.
     * 
     * @param confNo
     *            The conference to remove a member from
     * @param persNo
     *            The person to be removed from a converence
     */
    public void subMember(int confNo, int persNo) throws RpcFailure,
            IOException {
     ...
     * 
     * @param confNo
     *            The conference to add to the membership list
     * @param persNo
     *            The person who's membership list will be updated
     * @param prio
     *            The priority of the membership
     * @param listPos
     *            The position of this conference in the membership list
     * @param type
     *            A Bitstring containing the membership type information
     * @return An RpcCall object representing this specific RPC call
     */
    public RpcCall doAddMember(int confNo, int persNo, int prio, int listPos,
            Bitstring type) throws IOException {
     ...
     *            The position of this conference in the membership list
     * @param invititation
     *            If <tt>true</tt>, this is an invitation to become a member
     * @param passive
     *            If <tt>true</tt>, this membership is passive
     * @param secret
     *            If <tt>true</tt>, this membership is secret
     * @see nu.dll.lyskom.Session#joinConference(int)
     */
    public void addMember(int confNo, int persNo, int prio, int listPos,
            boolean invitation, boolean passive, boolean secret)
            throws IOException, RpcFailure {
        RpcReply reply = waitFor(doAddMember(
                confNo,
                persNo,
                prio,
                listPos,
                new MembershipType(
                        new boolean[] { invitation, passive, secret })).getId());
        if (!reply.getSuccess()) {
            throw reply.getException();
        }
     ...
     * 
     * @param confNo
     *            The conference (or letterbox/person) to change the name of
     * @param newName
     ...
     *            The new name of the conference
     */
    public void changeName(int confNo, String newName) throws IOException,
            RpcFailure {
        RpcCall call = null;
    ...
    /**
     * Sets the text number that contains a conference's or a person's
     * presentation.
     * 
     * @param confNo
     *            The conference for which to set the presentation
     * @param textNo
     *            The global text number containing the presentation
     */
    public void setPresentation(int confNo, int textNo) throws IOException,
            RpcFailure {
     ...
     * @return An RpcCall object representing this specific RPC call
     */
    public RpcCall doSetPresentation(int confNo, int textNo) throws IOException {
     ...
     * @see nu.dll.lyskom.Session#addMember(int, int, int, int, boolean,
     *      boolean, boolean)
     */
     ...
     * @return An RpcCall object representing this specific RPC call
     */
    public RpcCall doGetPersonStat(int persNo) throws IOException {
        RpcCall req = new RpcCall(count(), Rpc.C_get_person_stat).add(persNo);
    ...
    /**
     * Returns a <tt>Person</tt> object containing information about a given
     * person.
     * 
     * @param persNo
     *            The person to request information about
     * @param refreshCache
     *            If <tt>true</tt> don't look in the cache first.
     * @see nu.dll.lyskom.Person
     */
    public Person getPersonStat(int persNo) throws IOException, RpcFailure {
    ...
    /**
     * Returns a byte-array containing the name for a conference, or null if the
     * conference doesn't exist.
     * 
     * @param confNo
     *            conference number to retreive the name for
     */
    public byte[] getConfName(int confNo) throws IOException, RpcFailure {
        if (confNo == 0)
     ...
     * @return An RpcCall object representing this specific RPC call
     */
    public RpcCall doGetTextStat(int textNo) throws IOException {
    ...
    /**
     * Returns a <tt>TextStat</tt> object containing information about a given
     * text.
     * 
     * @param textNo
     *            The text number to request information about
     ...
     */
    public TextStat getTextStat(int textNo) throws IOException {
        return getTextStat(textNo, false);
    }

    /**
     * Returns a TextStat object for a given text number, or null if the text
     * doesn't exist or is inaccessible.
     * 
     * @param textNo
     *            The text number to request information about
     * @param refreshCache
     *            If <tt>true</tt>, a cached copy will not be returned
     * @see nu.dll.lyskom.TextStat
     */
     ...
     * 
     * This method does not store the RpcCall for user with a later call to
     * waitFor(), since the call is always expected to succeed.
     * 
     * @return An RpcCall object representing this specific RPC call
     */
    public RpcCall doUserActive() throws IOException {
        return writeRpcCall(new RpcCall(count(), Rpc.C_user_active), false);
    }

    public RpcCall doWhoAmI() throws IOException {
        return writeRpcCall(new RpcCall(count(), Rpc.C_who_am_i), true);
    }

    public int whoAmI() throws IOException, RpcFailure {
        if (mySessionNo > 0)
            return mySessionNo;
     ...
     * 
     * @param block
     *            If <tt>true</tt>, this method block until the server has
     *            confirmed the call
     */
    public void logout(boolean block) throws IOException, RpcFailure {
     ...
     * 
     * @param id
     *            The protocol request number to send to the server
     * @param s
     ...
     * 
     * @param t
     *            A Text object containing the information needed to construct
     *            the RPC call
     * @return An RpcCall object representing this specific RPC call
     * @see nu.dll.lyskom.Text
     */
    public RpcCall doCreateText(Text t) throws IOException {
        t.trimContents();
        ...
        if (!s.containsAuxItem(AuxItem.tagContentType)) {
            s.addAuxItem(new AuxItem(AuxItem.tagContentType,
                    "text/x-kom-basic; charset=" + getServerEncoding()));
        }
        return doCreateText(t.getContents(), s.getMiscInfo(), s.getAuxItems());
    }

    /**
     * Very simple reply function. Application developers are encouraged to use
     * createText() instead.
     */
    public int reply(int textNo, Text t) throws IOException {
     ...
     * 
     * @param text
     *            byte-array containing the article subject and text
     * @param miscInfo
     *            List of Selections with Misc-Info stuff (recipients, etc)
     * @param auxItems
     *            Auxiallry items (Aux-Item) attached to this article
     * @return An RpcCall object representing this specific RPC call
     * 
     */
    public RpcCall doCreateText(byte[] text, List<Selection> miscInfo,
            AuxItem[] auxItems) throws IOException {
    ...
    /**
     * Creates a text on the server, then returns the number of the newly
     * created text.
     * 
     * @param t
     *            A Text object containing the information needed to create the
     *            text
     ...
     * 
     * @param isConf
     *            If <tt>true</tt>, the refered object is a conference,
     *            otherwise a text
     * @param objNo
     *            If <tt>isConf</tt> is true: the conference to modify,
     *            otherwise the text number to modify
     * @param delAuxNo
     *            An array containing Aux-Info numbers to delete
     * @param addAux
     *            An array containing AuxItem objects to add
     * @see nu.dll.lyskom.AuxItem
     * @return An RpcCall object representing this specific RPC call
     */
    public RpcCall doModifyAuxInfo(boolean isConf, int objNo, int[] delAuxNo,
            AuxItem[] addAux) throws IOException, RpcFailure {
        ...
        for (int i = 0; i < addAux.length; i++) {
            ...
            Debug.println("doModifyAuxInfo(): adding aux-item " + i + ": "
                    + auxs[i]);
        }
     ...
     * @see nu.dll.lyskom.AuxItem
     */
    public void modifyAuxInfo(boolean isConf, int objNo, int[] delAuxNo,
            AuxItem[] addAux) throws IOException, RpcFailure {
        RpcReply reply = waitFor(doModifyAuxInfo(isConf, objNo, delAuxNo,
                addAux));
        if (!reply.getSuccess())
            throw reply.getException();
    }
    ...
    public RpcCall doSetPassword(int person, String userPassword,
    ...
    public void setPassword(int person, String userPassword, String newPassword)
            throws IOException, RpcFailure {
        RpcReply reply = waitFor(doSetPassword(person, userPassword,
                newPassword));
        if (!reply.getSuccess())
            throw reply.getException();
    }
    ...
    public RpcCall doAddRecipient(int textNo, int confNo, int type)
            throws IOException {
        RpcCall req = new RpcCall(count(), Rpc.C_add_recipient).add(
     ...
     * 
     * @param wantVisible
     *            If <tt>true</tt>, the server will return sessions marked as
     *            visible
     * @param wantInvisible
     *            If <tt>true</tt>, the server will return sessions marked as
     *            invisible
     * @param activeLast
     *            Do not return sessions that has been idle for more than
     *            <tt>activeLast</tt> seconds
     * @return An RpcCall object representing this specific RPC call
     */
    public synchronized RpcCall doWhoIsOnDynamic(boolean wantVisible,
            boolean wantInvisible, int activeLast) throws IOException {
        RpcCall req = new RpcCall(count(), Rpc.C_who_is_on_dynamic);
        req.add(wantVisible ? "1" : "0").add(wantInvisible ? "1" : "0")
                .add(activeLast);
        writeRpcCall(req);
        return req;
    }
    ...
    /**
     * Returns an array of DynamicSessionInfo objects containing information
     ...
     */
    public DynamicSessionInfo[] whoIsOnDynamic(boolean wantVisible,
            boolean wantInvisible, int activeLast) throws IOException {
        Debug.println("whoIsOnDynamic called");
        RpcReply reply = waitFor(doWhoIsOnDynamic(wantVisible, wantInvisible,
                activeLast).getId());

        KomToken[] parameters = reply.getParameters();
        DynamicSessionInfo[] ids = new DynamicSessionInfo[parameters[0]
                .intValue()];
        ...
        for (int i = 0, j = 5; i < ids.length; i++, j = j + 6)
            ids[i] = new DynamicSessionInfo(sessionData[j - 5].intValue(),
                    ...
                    sessionData[j - 2].intValue(), new Bitstring(
                            sessionData[j - 1]), sessionData[j].getContents());
    ...
    public RpcCall doDeleteText(int textNo) throws IOException {
        RpcCall req = new RpcCall(count(), Rpc.C_delete_text).add(new KomToken(
                textNo));
        writeRpcCall(req);
        return req;
    }
     ...
     * 
     * @param regexp
     *            A string containing the regexp that will be used for search
     * @param wantPersons
     *            Return persons matching <tt>name</tt>
     * @param wantConfs
     *            Return conferences matching <tt>name</tt>
     * @return An RpcCall object representing this specific RPC call
     * @see #lookupName(String, boolean, boolean)
     */
    public RpcCall doReLookup(String regexp, boolean wantPersons,
            boolean wantConfs) throws IOException {
    ...
    public ConfInfo[] reLookup(String regexp, boolean wantPersons,
            boolean wantConfs) throws IOException, RpcFailure {
        ...
        for (int i = 0, j = 2; i < ids.length; i++, j = j + 3)
            ids[i] = new ConfInfo((Hollerith) confData[j - 2], new ConfType(
                    confData[j - 1]), confData[j].intValue());
     ...
     * 
     * @param name
     *            (sub)string naming the person(s)/conference(s)
     * @param wantPersons
     *            Return persons matching <tt>name</tt>
     * @param wantConfs
     *            Return conferences matching <tt>name</tt>
    ...
    public ConfInfo[] lookupName(String name, boolean wantPersons,
        ...
        lcall.add(new Hollerith(name)).add(wantPersons ? "1" : "0")
                .add(wantConfs ? "1" : "0");
     ...
     * 
     * @param recipient
     *            The recipient conference, or <tt>0</tt> if it is a broadcast
     *            message
     * @param message
    ...
    public boolean sendMessage(int recipient, String message, boolean block)
            throws IOException, RpcFailure {
        RpcCall msgCall = new RpcCall(count(), Rpc.C_send_message).add(
                new KomToken(recipient)).add(new Hollerith(message));
    ...
    /**
     * Writes an RPC call constructed from an RpcCall object to the network
     * output stream.
     * 
     * @param c
     *            RpcCall object to be sent to the server
     * @param store
     *            if true, add it to the RPC call storage. This is required in
     *            order to use waitFor() etc
     */
    public RpcCall writeRpcCall(RpcCall c, boolean store) throws IOException {
        if (store)
        ...
        if (connection == null) {
            throw new IOException(
                    "Connection has gone away (see SF bug ID 973278).");
        }
    ...
    /**
     * Writes an RPC call constructed from an RpcCall object to the network
     * output stream and add it to the RPC call storage.
     * 
     * @param c
     *            RpcCall object to be sent to the server
     */
    public RpcCall writeRpcCall(RpcCall c) throws IOException {
        return writeRpcCall(c, true);
    }

    public void removeAttributes(String pattern) {
        synchronized (sessionAttributes) {
            for (Iterator<String> i = sessionAttributes.keySet().iterator(); i
                    .hasNext();) {
    ...
    public RpcReply waitFor(int id) throws IOException {
        return waitFor(singleton(id));
    }

    public Collection<Integer> singleton(int i) {
        HashSet<Integer> h = new HashSet<Integer>();
        h.add(new Integer(i));
        return h;
    }

    public RpcReply waitFor(Collection<Integer> ids) throws IOException {
    ...
    public RpcCall waitForCall(RpcCall call) throws IOException {
        return waitForCall(call.getId());
    }

    public RpcCall waitForCall(int id) throws IOException {
    ...
    /**
     * This methods provides a synchronous way of waiting for RPC replies. Note
     * that this method should never be called from the MessageListener thread
     * (for example, you may not call this method from inside an
     * AsynchMessageListener's asynchMessage() method). <br>
     * It blocks the current thread until a reply with the specified reference
     * ID has been read from the server, or the rpcTimeout value has been
     * reached.
     * 
     * @param id
     ...
     * 
     */
    public RpcCall waitForCall(Collection<Integer> ids) throws IOException {
        ...
        while (call == null) {
            ...
            try {
                ...
                // do a check before the wait() in case the reply has arrived
                // already
                if (null == (call = rpcHeap.getRpcCall(ids, true))) {
                    ...
                    synchronized (rpcHeap) {
                        rpcHeap.wait(rpcSoftTimeout > 0 ? rpcSoftTimeout
                                : rpcTimeout);
                    }
                } else {
                ...
                Debug.println("RPC waitForCall() interrupted: "
                        + ex1.getMessage());
                // ...?
            }
            call = rpcHeap.getRpcCall(ids, true);
            if (call == null && listener != null
                    && listener.getException() != null) {
    ...
    public boolean fallbackCheck(Collection<Integer> ids, RpcCall call)
            throws IOException {
        RpcReply reply = call.getReply();
        if (!reply.getSuccess()
    ...
    void updateCachesNewText(TextStat textStat) {
        List<Selection> miscInfo = textStat.getMiscInfo();
        for (int i = 0; i < miscInfo.size(); i++) {
            Selection misc = miscInfo.get(i);
            int key = misc.getKey();
            if (key == TextStat.miscRecpt || key == TextStat.miscCcRecpt
                ...
                UConference cachedUConf = conferenceCache
                        .getUConference(recipient);
                if (cachedUConf != null) {
                ...
                try {
                    if (unreadsConferences != null) {
                        synchronized (unreadsConferences) {
                            if (!unreadsConferences.contains(recipientObj)
                                    && !readTexts.contains(textStat.getNo())
                                    && isMemberOf(recipient, false)) {
                                unreadsConferences.add(recipientObj);
                            }
                        }
                    }
                } catch (IOException ex1) {
    ...
    private void acceptAsynchAll() throws IOException, RpcFailure {
        acceptAsync(new int[] { Asynch.new_name, Asynch.sync_db,
                Asynch.leave_conf, Asynch.login, Asynch.rejected_connection,
                Asynch.send_message, Asynch.logout, Asynch.deleted_text,
                Asynch.new_text, Asynch.new_recipient, Asynch.sub_recipient,
                Asynch.new_membership });
    }

    public void acceptAsync(int[] requestList) throws IOException, RpcFailure {
    ...
    public void asynchMessage(AsynchMessage m) {
        ...
        case Asynch.new_recipient:
            updateCachesNewRecipient(parameters[0].intValue(),
                    parameters[1].intValue(), parameters[2].intValue());
            break;
